...
<form action="?page=salvar-paguamento" method="POST">
    ...
    <div class="mb-3">
        <label>Agendamento
            <select name="agendamento_id" id="agendamento_id" class="form-control" required onchange="preencherValor()">
                <option value="">-= Selecione o Agendamento =-</option>
                <?php
                $sql = "SELECT ag.*, cli.nome_cliente, srv.nome_servico, srv.preco_servico, barb.nome_barbeiro 
                ...
                if ($res && $res->num_rows > 0) {
                    while ($row = $res->fetch_object()) {
                        ...
                        print "<option value='{$row->id_agendamento}' data-preco='{$row->preco_servico}'>{$row->nome_cliente} - {$row->nome_servico} - {$data_formatada}</option>";
                    }
                } else {
    ...
    <div class="mb-3">
        <label>Valor (R$)
            <input type="number" step="0.01" name="valor" id="valor" class="form-control" placeholder="Ex: 50.00"
                required>
        </label>
    </div>
